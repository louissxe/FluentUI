...
local aa = {
    function()
        ...
        function SaveManager:BuildConfigSection(tab)
            ...
            sec:AddInput("SaveManager_ConfigName",{Title="Config name"})
            sec:AddDropdown("SaveManager_ConfigList",{Title="Config list",Values=self:RefreshConfigList(),AllowNull=true,NoSearch=true})
            sec:AddButton({Title="Create config",Callback=function()
            ...
            sec:AddButton({Title="Overwrite config",Callback=function()
                local name=SaveManager.Options.SaveManager_ConfigList.Value
                local ok,err=self:Save(name)
                if not ok then return self.Library:Notify({Title="Interface",Content="Config loader",SubContent="Failed: "..err,Duration=7}) end
                self.Library:Notify({Title="Interface",Content="Config loader",SubContent=string.format("Overwrote %q",name),Duration=7})
            end})
            sec:AddButton({Title="Refresh list",Callback=function()
                SaveManager.Options.SaveManager_ConfigList:SetValues(self:RefreshConfigList())
                SaveManager.Options.SaveManager_ConfigList:SetValue(nil)
            end})
            local _autoBtn
            _autoBtn=sec:AddButton({Title="Set as autoload",Description="Current autoload: none",Callback=function()
                local name=SaveManager.Options.SaveManager_ConfigList.Value
        ...
        function InterfaceManager:BuildInterfaceSection(tab)
            ...
            if Library.UseAcrylic then
                section:AddToggle("AcrylicToggle",{Title="Acrylic",Description="Requires graphic quality 8+",Default=Settings.Acrylic,Callback=function(Value)
                    Library:ToggleAcrylic(Value); Settings.Acrylic=Value; InterfaceManager:SaveSettings()
                end})
            end
            section:AddToggle("TransparentToggle",{Title="Transparency",Description="Makes the interface transparent.",Default=Settings.Transparency,Callback=function(Value)
                Library:ToggleTransparency(Value); Settings.Transparency=Value; InterfaceManager:SaveSettings()
            end})
            section:AddToggle("AnimationToggle",{Title="Animated Theme",Description="Enables shine/stroke animation on theme.",Default=Settings.Animated,Callback=function(Value)
        ...
        function FloatingButtonManager:BuildConfigSection(tab)
            ...
            sec:AddInput("FB_ConfigName",{Title="Layout name"})
            sec:AddDropdown("FB_ConfigList",{Title="Layouts",Values=self:RefreshConfigList(),AllowNull=true,NoSearch=true})
            sec:AddButton({Title="Save layout",Callback=function()
            ...
            sec:AddButton({Title="Load layout",Callback=function()
            ...
            sec:AddButton({Title="Overwrite layout",Callback=function()
                local name=self.Library.Options.FB_ConfigList.Value
                if not name or name == "" then return self.Library:Notify({Title="Floating Buttons",Content="No layout selected",Duration=5}) end
                local ok,err=self:Save(name)
                if not ok then return self.Library:Notify({Title="Floating Buttons",Content="Failed: "..err,Duration=5}) end
            ...
            local _autoFBBtn
            _autoFBBtn=sec:AddButton({Title="Set as autoload",Description="Current autoload: none",Callback=function()
                local name=self.Library.Options.FB_ConfigList.Value
                if not name or name == "" then return self.Library:Notify({Title="Floating Buttons",Content="No layout selected",Duration=5}) end
                writefile(self.Folder.."/settings/autoload.txt",name)
